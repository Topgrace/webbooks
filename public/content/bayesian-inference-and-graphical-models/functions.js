...
var StepFunctions = (() => {
  ...
  // content/bayesian-inference-and-graphical-models/functions.ts
  ...
  // content/bayesian-inference-and-graphical-models/functions.ts
  function pdfBeta(\u03B1, \u03B2) {
    return (p) => 1 / beta(\u03B1, \u03B2) * p ** (\u03B1 - 1) * (1 - p) ** (\u03B2 - 1);
  }
  function betadensity($step) {
  ...
  var GAMMA_CONST = 0.5772156649015329;
  var P_COFF = [
    -1.716185138865495,
    24.76565080557592,
    -379.80425647094563,
  ...
  function gamma2(x) {
    if (x <= 0) {
      ...
      return 1 / (x * (1 + GAMMA_CONST * x));
    } else if (x < 12) {
      var y = x, n = 0, lessOne = y < 1;
      if (lessOne) {
        y += 1;
      } else {
        n = Math.floor(y) - 1;
  ...
  var HALF_LOG_TWO_PI = 0.9189385332046728;
  function logGamma(x) {
    if (x <= 0) {
      throw new RangeError("Argument must be positive.");
    } else if (x < 12) {
  ...
  function beta(x, y) {
    if (x < 0 || y < 0) {
      ...
      return gamma2(x) * gamma2(y) / gamma2(x + y);
    }
  }
