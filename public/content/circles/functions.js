...
var StepFunctions = (() => {
  ...
  function replaceSvgImports() {
    ...
    uses.forEach(function(use) {
      ...
      svg.removeChild(use);
      if (!(url2 in requests))
        requests[url2] = fetch(url2).then((r) => r.text());
  ...
  function post(url2, data) {
    return __async2(this, null, function* () {
      ...
      if (!response.ok)
        throw new Error(`Fetch error ${response.status}: ${url2}`);
  ...
  function create3D(_0, _1, _2) {
    return __async(this, arguments, function* ($el, fov, width2, height2 = width2, camera) {
      ...
      if (!camera)
        camera = new THREE.PerspectiveCamera(fov, width2 / height2, 0.1, 1e3);
      scene.add(camera);
      const callbacks = [];
    ...
    ready() {
      ...
      $geopad.model.watch((s) => {
        ...
        if (d > stringLength + threshold)
          pen.setValue(ellipse.project(p));
      });
      let lastC = pen.value;
