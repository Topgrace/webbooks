...
var StepFunctions = (() => {
  ...
  // content/intro-statistics/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    gaussiandensity: () => gaussiandensity,
    gaussiandensity_mle: () => gaussiandensity_mle,
    kdecrossvalidate: () => kdecrossvalidate,
    mleCaution: () => mleCaution,
    tricubegraph: () => tricubegraph
  });
  ...
  function calculate(eventRoot, selfIntersection) {
    ...
    while (eventRoot.head) {
      ...
      if (ev.isStart) {
        ...
        const above = (_a2 = surrounding.before) == null ? void 0 : _a2.ev;
  ...
  // content/shared/components/quill.ts
  var promise;
  var MAX_LENGTH = 2500;
  ...
  var QuillComponent = class extends CustomElementView {
    setup($step, goal, userData) {
      return __async(this, null, function* () {
        var _a2;
        yield loadEditorAssets();
        ...
        let initialText = ((_a2 = userData == null ? void 0 : userData.data) == null ? void 0 : _a2.quill) || "";
        if (initialText)
          this.quill.setText(initialText);
        this.quill.on("text-change", () => {
          const length = this.quill.getLength();
          if (length > MAX_LENGTH)
            this.quill.deleteText(MAX_LENGTH, length);
        });
  ...
  QuillComponent = __decorateClass([
    register("x-quill")
  ], QuillComponent);

  // content/shared/components/chart.ts
  var FUNCTION_STEP = 2;
  var TICK_LENGTH = 4;
  var MARKER = `<marker id="axis-arrow" refX="3" refY="3" markerWidth="6"
    markerHeight="6" orient="auto"><path d="M 0 0 L 6 3 L 0 6 z"/></marker>`;
  var CoordinateSystem = class extends CustomElementView {
    ready() {
      const w = +this.attr("width");
      const h = +this.attr("height");
      this.$svg = $N("svg", { width: w, height: h, viewBox: `0 0 ${w} ${h}`, class: "canvas" }, this);
      $N("defs", { html: MARKER }, this.$svg);
      ...
      this.$xAxis = $N("line", { x1: vB.xMin, x2: vB.xMax, y1: origin.y, y2: origin.y }, this.$axes);
      this.$yAxis = $N("line", { x1: origin.x, x2: origin.x, y1: vB.yMax, y2: vB.yMin }, this.$axes);
  ...
  CoordinateSystem = __decorateClass([
    register("x-chart")
  ], CoordinateSystem);

  // content/shared/shared.ts
  ...
  function pdf(m, s) {
    return (x) => 1 / (Math.sqrt(2 * Math.PI) * s) * Math.exp(-((x - m) ** 2) / (2 * s ** 2));
  }
  function gaussiandensity($step) {
    const $chart = $step.$("x-chart");
    const xs = [
      71.54,
      66.62,
      64.11,
  ...
  function tricube(l) {
    return (u) => 1 / l * (Math.abs(u / l) < 1 ? 70 / 81 * (1 - Math.abs(u / l) ** 3) ** 3 : 0);
  }
  function tricubegraph($step) {
    const $chart = $step.$("x-chart");
    $step.model.watch((s) => {
  ...
  function estimator(xs, i, l) {
    const fn = tricube(l);
    return (x) => xs.filter((_, j) => j != i - 1).map((xi) => fn(x - xi)).reduce((a, b) => a + b, 0) / 5;
  }
  function kdecrossvalidate($step) {
    const $chart = $step.$("x-chart");
  ...
  function gaussiandensity_mle($step) {
    const $chart = $step.$("x-chart");
    const xs = [
      71.54,
      66.62,
  ...
  function twoStep(a, b, c, d, g) {
    const delta = (1 - g * (b - a)) / (d - c);
    return (x) => a < x && x < b ? g : c < x && x < d ? delta : 0;
  }
  function mleCaution($step) {
    const $chart = $step.$("x-chart");
