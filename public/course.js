...
(() => {
  ...
  function list(a2, b2, step = 1) {
    const arr = [];
    if (b2 === void 0 && a2 >= 0) {
      for (let i = 0; i < a2; i += step)
        arr.push(i);
    } else if (b2 === void 0) {
      for (let i = 0; i > a2; i -= step)
        arr.push(i);
    } else if (a2 <= b2) {
      for (let i = a2; i <= b2; i += step)
        arr.push(i);
    } else {
      for (let i = a2; i >= b2; i -= step)
        arr.push(i);
  ...
  function subsets(array, length = 0) {
    ...
    const results = subsetsHelper(copy2);
  ...
  function calculate(eventRoot, selfIntersection) {
    ...
    while (eventRoot.head) {
      ...
      if (ev.isStart) {
        ...
        const above = (_a4 = surrounding.before) == null ? void 0 : _a4.ev;
  ...
  var Polygon = class _Polygon {
    ...
    cut(line) {
      ...
      const mask = [a2, b2, b2.add(d), a2.add(d)];
  ...
  var BrowserInstance = class {
    ...
    getActiveInput() {
      ...
      if (active2 == null ? void 0 : active2.shadowRoot)
  ...
  function hover($el, options) {
    ...
    $clickTarget.on("focus", () => {
      if (active2 || options.preventMouseover && options.preventMouseover())
        return;
      clearTimeout(timeout);
      enter2();
  ...
  function copySVGStyles(source, copy2, isHTML = false) {
    ...
    for (const p of properties) {
      ...
      const inherited = inheritedStyle(copy2, p);
      if (value === STYLE_DEFAULTS[p] && !inherited)
        continue;
      if (!DONT_INHERIT.has(p) && value === inherited)
        continue;
      copy2.style.setProperty(p, value);
    }
    const sourceChildren = source.children;
    const copyChildren = copy2.children;
    ...
    onKey(keys, callback, options) {
      ...
      target.addEventListener(event, (e) => {
        ...
        if (!key || !keyNames.has(key))
          return;
        if (document.activeElement !== this._el && ((_b = (_a22 = document.activeElement) == null ? void 0 : _a22.shadowRoot) == null ? void 0 : _b.activeElement) !== this._el && Browser.formIsActive)
          return;
        callback(e, key);
      });
    }
    ...
    image(type, width2, height, viewBox) {
      return __async2(this, null, function* () {
        ...
        if (type === "svg")
          cleanSVG($copy._el);
        if (!height)
          height = width2 || this.svgHeight;
        if (!width2)
          width2 = this.svgWidth;
  ...
  var Draggable = class extends EventTarget {
    ...
    setPosition(x, y) {
      ...
      if (this.bounds)
        p = p.clamp(this.bounds, (_a22 = this.options.margin) != null ? _a22 : 0);
    ...
    setup(options = {}) {
      ...
      if (options.history) {
        window.addEventListener("popstate", (e) => __async2(this, null, function* () {
          var _a22;
          if (!isReady2 || !((_a22 = e.state) == null ? void 0 : _a22.path))
            return;
      ...
      for (const [i, p] of this.paragraphs.entries()) {
        p.on("end", () => {
          ...
          if (this.paragraphs[i + 1]) {
            ...
            if ($next && $next.isShown)
              (_a4 = $next.narration) == null ? void 0 : _a4.play();
          }
        });
      }
    }
      ...
      for (const [i, s] of this.sentences.entries()) {
        s.on("end", (playNext) => {
          ...
          if (playNext && this.sentences[i + 1]) {
            ...
            if (playNext)
              setTimeout(() => this.trigger("end"), 400);
          }
        });
      }
    }
  ...
  var Modal = class extends CustomElementView {
    ...
    open(noAnimation = false) {
      ...
      if ($focus)
        $focus.focus();
      (_a4 = window.ga) == null ? void 0 : _a4.call(window, "send", "event", "Modal", this.id);
  ...
  var Gesture = class extends CustomElementView {
    ...
    start(slide2) {
      if (this.doAnimation || !this.from && !this.$target)
        return;
  ...
  var Gloss = class extends CustomElementView {
    ...
    body() {
      ...
      if (!data)
        return console.warn("missing gloss:", this.xid);
      let body = data.text;
      if (data.image)
        body += `<img class="gloss-img" alt="" src="/content/shared/glossary/${data.image}"/>`;
      if (data.link && true) {
        const samePage = location.pathname === data.link.split("#")[0];
        if (!samePage)
          body += `<p><a href="${data.link}" class="btn btn-white btn-small">Learn more\u2026</a></p>`;
      }
      return body;
    }
  };
  ...
  var Bio = class extends Gloss {
    body() {
      ...
      if (!data)
        return console.warn("missing bio:", this.xid);
  ...
  var Picker = class extends CustomElementView {
    ...
    setup($step, id, userData) {
      ...
      for (const [i, $item] of $items.entries()) {
        ...
        if (!error)
          this.correctCount += 1;
        $item.one("click", () => {
          if (itemsClicked[i] || this.isSolved)
            return;
    ...
    checkSolved() {
      if (this.solvedCount < this.correctCount)
        return;
  ...
  var Slider = class extends CustomElementView {
    ...
    ready() {
      ...
      this.drag.on("move", ({ posn }) => {
        ...
        if (!continuous)
          n = Math.round(n);
        if (nearlyEquals(n, this.current))
          return;
        this.current = n;
        this.trigger("move", n);
      });
    ...
    set(x) {
      if (nearlyEquals(x, this.current))
        return;
    ...
    moveTo(x, duration = 600) {
      return __async(this, null, function* () {
        if (this.animation || x === this.current)
          return;
  ...
  var Slideshow = class extends CustomElementView {
    ...
    ready() {
      ...
      this.reveals = this.$$("[slide]").map(($el) => {
        ...
        if (range2[0] >= 0 && !$el.hasClass("reveal"))
          $el.hide();
    ...
    go(x) {
      if (this.locked || x < 0 || x > this.length - 1 || x === this.current)
        return;
      this.locked = true;
  ...
  function position(items, except) {
    ...
    for (const [i, item] of items.entries()) {
      ...
      if (item !== except)
        item.drag.setPosition(0, cumHeights[i - 1] || 0);
    }
  }
  ...
  var Step = class extends CustomElementView {
    ...
    show() {
      ...
      this.addClass("on");
      if (this.narration)
        setTimeout(() => this.narration.play(), 400);
    }
    complete() {
      if (this.isCompleted)
        return;
    ...
    score(goal, goNext = true) {
      ...
      if (goNext && this.isReady && this.$course && this.$course.isReady) {
        setTimeout(() => {
          if (this.$course.$activeStep === this)
            this.$course.nextStep();
        }, 1200);
      }
    }
    ...
    addHint(text, options = {}) {
      ...
      if (!((_a4 = this.$course) == null ? void 0 : _a4.isReady))
        return { text };
    ...
    groupBlanks(...indices) {
      ...
      for (const b2 of blanks)
        b2.linkedBlanks = blanks;
    }
  };
  ...
  function pow(a2, b2) {
    if (a2[0] > 0) {
      if (b2[0] >= 0)
        return int(a2[0] ** b2[0], a2[1] ** b2[1]);
      return range(a2[0] ** b2[0], a2[0] ** b2[1], a2[1] ** b2[0], a2[1] ** b2[1]);
    }
    const k = b2[0];
    if (Number.isInteger(k) && k === b2[1]) {
      if (k === 0)
        return [hasZero(a2) ? 0 : 1, 1];
      if (k % 2)
        return int(a2[0] ** k, a2[1] ** k);
      if (hasZero(a2))
        return [0, Math.max(a2[0] ** k, a2[1] ** k)];
  ...
  var interval = {
    ...
    abs: (a2) => {
      if (hasZero(a2))
        return int(0, Math.max(-a2[0], a2[1]));
    ...
    log: (a2, b2) => {
      if (b2 !== void 0)
        interval.div(interval.log(a2), interval.log(b2));
    ...
    cos: (a2) => {
      if (isEmpty(a2) || isInfinite(a2))
        return EMPTY2;
      if (width(a2) >= TWO_PI2 - Number.EPSILON)
        return [-1, 1];
      a2 = intervalMod(a2);
      if (a2[0] > Math.PI + Number.EPSILON)
        return interval.sub(interval.cos(interval.sub(a2, [Math.PI, Math.PI])));
      if (a2[1] < Math.PI - Number.EPSILON)
        return int(Math.cos(a2[1]), Math.cos(a2[0]));
      if (a2[1] < TWO_PI2 - Number.EPSILON)
        return int(-1, Math.max(Math.cos(a2[1]), Math.cos(a2[0])));
      return int(-1, 1);
    },
    tan: (a2) => {
      if (isEmpty(a2) || isInfinite(a2))
      ...
      if (a2[0] > HALF_PI + Number.EPSILON)
        a2 = interval.sub(a2, [Math.PI, Math.PI]);
      if (a2[0] < -HALF_PI + Number.EPSILON || a2[1] > HALF_PI - Number.EPSILON)
        return WHOLE;
    ...
    cosh: (a2) => {
      if (a2[1] < 0)
        return int(Math.cosh(a2[1]), Math.cosh(a2[0]));
      if (a2[0] > 0)
        return int(Math.cosh(a2[0]), Math.cosh(a2[1]));
  ...
  var ExprFunction = class _ExprFunction extends ExprElement {
    ...
    evaluate(vars = {}) {
      ...
      if (this.fn in vars) {
        ...
        if (typeof fn === "number" && args.length === 1)
          return evaluate2.mul(fn, args[0]);
        throw ExprError.uncallableExpression(this.fn);
      }
      if (this.fn === "+")
        return evaluate2.add(...args);
      if (this.fn === "\u2212")
        return evaluate2.sub(...args);
      if (["*", "\xB7", "\xD7"].includes(this.fn))
        return evaluate2.mul(...args);
      if (this.fn === "/")
        return evaluate2.div(...args);
      if (this.fn === "sup")
        return evaluate2.sup(...args);
      if (isSpecialFunction(this.fn))
        return evaluate2[this.fn](...args);
      if (this.fn === "(")
        return args[0];
    ...
    interval(vars = {}) {
      ...
      if (this.fn in vars) {
        const fn = vars[this.fn];
        if (typeof fn === "function")
          return repeat(fn(...args.map((a2) => a2[0])), 2);
        if (typeof fn === "number" && args.length === 1)
  ...
  var Tutor = class extends CustomElementView {
    ...
    ready() {
      ...
      if (showWelcome && this.$course) {
        if (true)
          Browser.setCookie("sessionWelcome", 1, 60 * 60 * 4);
        ...
        if (user) {
          ...
          if (true)
            setTimeout(() => this.queue(this.hints.account), 4500);
        } else {
          Browser.setCookie("welcome", 1);
          setTimeout(() => this.queue(this.hints.welcome), 3e3);
          if (true)
            setTimeout(() => this.queue(this.hints.tutorial1), 4500);
        }
      }
    }
    // ---------------------------------------------------------------------------
    ...
    askQuestion(query) {
      ...
      post(this.attr("api"), { query }).then((res) => {
        ...
        for (const msg of data)
          this.queue(msg.content, msg.kind);
      }).catch((error) => {
        this.$chatBody.removeClass("loading");
  ...
  var Video = class extends CustomElementView {
    ready() {
      ...
      if (this.hasAttr("loop"))
        $video._el.loop = true;
      if (!this.hasAttr("audio"))
        $video._el.muted = true;
  ...
  var Course = class extends CustomElementView {
    ...
    created() {
      ...
      for (const $s of this.$steps)
        $s.on("score", () => this.trigger("score"));
      if (this.data.audio)
        this.audio = new AudioSegment(this.data.audio);
    }
    ready() {
    ...
    goToStep($step, animated = true) {
      ...
      if ($last.isShown && $last.isReady)
        return this.complete();
      if (this.$activeStep)
        this.saveProgress({ activeStep: this.$activeStep.id });
    }
