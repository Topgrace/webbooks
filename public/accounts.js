...
(() => {
  ...
  for (let i = 0; i < 2; ++i)
    combinations.push(combinations[i].toUpperCase());
  for (let i = 0; i < 5; ++i) {
  ...
  RouterInstance.view("/signup", {
    enter($el) {
      ...
      $bday.change((date) => {
        ...
        if (age < 0 || age > 100)
          return $bday.setValidity("Please enter a valid date of birth.");
        if (age < 13)
          return $bday.setValidity("You have to be at least 13 years old to create an account.");
        $bday.setValidity("");
      });
    }
  });
