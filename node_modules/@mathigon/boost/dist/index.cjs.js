...
function makeTapEvent($el) {
  ...
  $el.on("pointerup", (e) => {
    ...
    if (import_euclid.Point.distance(start, end) < 6)
      $el.trigger("tap", e);
    start = void 0;
  });
...
function slide($el, fns) {
  ...
  function move(e) {
    ...
    e.preventDefault();
    const p = posn(e);
    if (import_euclid.Point.distance(p, lastPosn) < 0.5)
      return;
...
var BaseView = class {
  ...
  addClass(className) {
    for (const c of (0, import_core5.words)(className))
  ...
  setTransform(posn, angle = 0, scale = 1) {
    let t = "";
    if (posn)
      t += `translate(${(0, import_fermat.roundTo)(posn.x, 0.1)}px,${(0, import_fermat.roundTo)(posn.y, 0.1)}px)`;
    if (angle)
      t += ` rotate(${angle}rad)`;
    if (scale)
      t += ` scale(${scale})`;
  ...
  hasParent(...$p) {
    ...
    while (parent) {
      if ((0, import_core5.isOneOf)(parent, ...tests))
        return true;
...
var InputView2 = class extends HTMLBaseView {
  ...
  bindVariable(model, name) {
    ...
    if (this._el.type === "number") {
      ...
      this.change((v) => {
        if (v)
          model[name] = (0, import_fermat.clamp)(+v, min, max);
      });
    } else {
