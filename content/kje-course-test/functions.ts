...
export function similar($step: Step) {
  ...
  const burst = new Burst($N('g', {class: 'burst'}, $svg) as SVGParentView, 20);//burst효과
  const $gesture = $N('x-gesture', {}, $step) as Gesture;

  $N('circle', {class: 'handle fixed', r: 10, cx: 320, cy: 180, target: 'greenCircle', style: 'fill: #22ab24'}, $svg);//타겟중심점
  $N('circle', {class: 'outline', r: 60, cx: 320, cy: 180, target: 'greenCircle',style: 'stroke: #22ab24'}, $strokes);//타겟원
  ...
  for (const c of circles) {
    ...
    const $handle = $N('circle', {class: 'handle',target: 'redCircle', r: 10}, $svg);
    const $outlineHalo = $N('circle', {class: 'outline-halo', r: c[2]}, $strokes) as SVGView;
    const $outline = $N('circle', {class: 'outline', target: 'redCircle',r: c[2]}, $strokes) as SVGView;
